...
default_synthesis_model_kwargs = {'source_sed': 'eldridge2009',\
    'source_Z': 0.0245, 'interp_Z': 'linear', 'verbose': False}

class AresSignalModelWithUVLF(LoadableModel):
    """
    Model subclass wrapper around the ares.simulations.Global21cm object.
    """
    def __init__(self, frequencies, magnitudes, parameters=[], in_Kelvin=False,\
        ...
        return_joint_signal_UVLF=True, add_popIII_stars_to_UVLF=False):
        """
        ...
        magnitudes: UV Magnitudes at which to evaluate the LuminosityFunction
        ...
        return_joint_signal_UVLF: return the global signal with the UVLF 
                                  concatenated to the end. Default True.
        """
        ...
        self.debug = debug
        self.return_joint_signal_UVLF = return_joint_signal_UVLF
        self.add_popIII_stars_to_UVLF = add_popIII_stars_to_UVLF
    ...
    def magnitudes(self):
        """
        Property storing the 1D UV Luminosity Magnitudes array at which the signal model
        should apply.
        """
        if not hasattr(self, '_magnitudes'):
            raise AttributeError("magnitudes was referenced before it was " +\
                "set.")
    ...
    def magnitudes(self, value):
        """
        Setter for the UV Luminosity Magnitudes at which this model should apply.
        ...
        if type(value) in sequence_types:
            ...
            raise TypeError("magnitudes was set to a non-sequence.")
    ...
    def ares_kwargs(self):
        ...
        if not hasattr(self, '_ares_kwargs'):
            ...
            #TODO Add back in the hmf_instance, and tau
            ...
            print('Called ares_kwargs in SignalModel')
    ...
    def __call__(self, parameters):
        ...
        simulation_UVLF = simulation.pops[0].LuminosityFunction(5.9, self.magnitudes)[1]
        if self.add_popIII_stars_to_UVLF:
        	popIII_UVLF = simulation.pops[2].LuminosityFunction(5.9, self.magnitudes)[1]
        	simulation_UVLF = simulation_UVLF + popIII_UVLF
        ...
        if self.in_Kelvin:
            gc.collect()
            signal = signal / 1e3
        if self.return_joint_signal_UVLF:
            joint_signal_UVLF = np.concatenate((signal, simulation_UVLF), axis=0)
            return joint_signal_UVLF
        else:
            return simulation_UVLF
