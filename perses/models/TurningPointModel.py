"""
File: perses/models/TurningPointModel.py
...
class TurningPointModel(LoadableModel):
    """
    ...
    def __call__(self, parameters):
        ...
        known_temperatures =\
            np.repeat(np.concatenate([[0], parameters[5:], [0]]), 2)
        ...
        if parameters[4] < self.frequencies[-1]:
            signal_in_mK[self.frequencies > parameters[4]] = 0
        if self.in_Kelvin:
